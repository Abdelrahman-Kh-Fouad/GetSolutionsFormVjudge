...
ll dp2(vector<vector<char>>&arr , int x =0  , int y = 0 )
{
    ...
    ll &stat = memo2[x][y];
    if(stat !=-1) return stat ; 
    ll ch1 = valid(arr , x+1 , y )? arr[x+1 ][y ] ='6' , dp2(arr ,x+1 , y ) , arr[x+1][y] ='.': 0;
    ll ch2 = valid(arr , x , y+1 )? arr[x ][y+1 ] ='6' , dp2(arr ,x , y+1 ) , arr[x][y+1] ='.': 0;
    ll ch3 = valid(arr , x-1 , y )? arr[x-1 ][y] ='6' , dp2(arr ,x-1 , y ) , arr[x-1][y] ='.': 0;
    ll ch4 = valid(arr , x , y-1 )? arr[x][y-1 ] ='6' , dp2(arr ,x , y-1 ) , arr[x][y-1] ='.': 0;
    return stat = (ch1 %MOD +ch2 %MOD + ch3 %MOD  +ch4 %MOD  )%MOD;
}
int main()
{
